...
#      through geometry at low alpha, colour-coded by role.
#
# Signal integrity gets equal billing with health because a robot at full HP and
# 0.2 signal is functionally out of the fight — that needs to be legible at a
# glance or the e-warfare system is invisible to the player.
# ─────────────────────────────────────────────
...
const COL_POSSESS := HUDPalette.POSSESS
...
func _ready() -> void:
	...
	# The HUD's GreenFilter ColorRect is a full-screen sibling; later siblings
	# draw on top, so sit last or the filter buries the panel.
	z_index = 50
	_autowire()
	_build_ui()
	...
	if possession != null:
		possession.possessed_body.connect(_on_possessed)
		possession.released_body.connect(_on_released)
		possession.possession_failed.connect(_on_possession_failed)
...
func _autowire() -> void:
	...
	if possession == null:
		possession = _find_first(get_tree().root, "PossessionController")
	if player == null:
		if commander != null:
			player = commander.player
		elif possession != null:
...
func _build_ui() -> void:
	...
	# Verb wheel, centred just under the crosshair.
	# Anchored to the same bottom-left corner as the roster panel and then
	# offset from the panel's top-left, so wheel_offset shifts it relative to
	# the roster rather than relative to screen centre.
...
func _refresh_roster() -> void:
	...
	var ctx := "CONTACT" if squad.context == Squad.SquadContext.ENGAGED else "CLEAR"
	...
	_squad_header.add_theme_color_override("font_color",
		COL_WARN if squad.context == Squad.SquadContext.ENGAGED else COL_BRIGHT)
...
func _make_member_row(m: Soldier) -> Control:
	...
	var possessed_here: bool = possession != null \
		and possession.is_possessing() \
		and possession.possessed == m
 
	var name_col := COL_DIM
	if not m.alive:
		name_col = COL_CRIT
	elif possessed_here:
		name_col = COL_POSSESS
	elif m.alive:
		name_col = COL_BRIGHT
 
	var tag := ">" if possessed_here else " "
	var name_label := _make_label("%s%-10s" % [tag, m.soldier_name.left(10)], name_col)
	...
	var state := _state_text(m, possessed_here)
	row.add_child(_make_label(state, COL_DIM))
	return row
...
func _state_text(m: Soldier, possessed_here: bool) -> String:
	if possessed_here:
		return "MANUAL"
...
func _draw() -> void:
	...
	for m in squad.get_living_members():
		...
		var col := COL_BRIGHT
		if possession != null and possession.possessed == m:
			col = COL_POSSESS
		col.a = alpha
...
func _on_possessed(soldier: Soldier) -> void:
	_show_toast("CONTROL ASSUMED : %s" % soldier.soldier_name.to_upper(), COL_POSSESS)
 
 
func _on_released(soldier: Soldier) -> void:
	var who := soldier.soldier_name.to_upper() if is_instance_valid(soldier) else "UNIT"
	_show_toast("CONTROL RELEASED : %s" % who, COL_DIM)
 
 
func _on_possession_failed(reason: String) -> void:
	_show_toast(reason, COL_CRIT)
